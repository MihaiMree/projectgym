...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $stmt = $conn->prepare('INSERT INTO bi_cable_one_hamer(naam, gewicht_set1, reps_set1, gewicht_set2, reps_set2, gewicht_set3, reps_set3, date) VALUES (?, ?, ?, ?, ?, ?, ?,?)');
...
if (!empty($message)) {
    header("Location: /projectGYM/biceps.php");
    exit();
}
?>
<?php
